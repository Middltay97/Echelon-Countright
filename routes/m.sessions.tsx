...
export const Route = createFileRoute("/m/sessions")({
  component: Sessions,
  errorComponent: SessionsError,
});

function SessionsError({ error, reset }: { error: Error; reset: () => void }) {
  if (recoverFromChunkLoadError(error)) return null;
  return (
    <div className="min-h-dvh bg-background flex items-center justify-center px-6">
      <div className="max-w-sm text-center">
        <h1 className="text-lg font-semibold text-foreground">Sessions couldn't load</h1>
        <p className="mt-2 text-sm text-muted-foreground break-words">{error.message}</p>
        <div className="mt-4 flex justify-center gap-2">
          <button onClick={reset} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground">Try again</button>
          <a href="/m" className="rounded-md border border-border bg-card px-3 py-2 text-sm font-medium text-foreground">Back to sign-in</a>
        </div>
      </div>
    </div>
  );
...
function Sessions() {
  ...
  const load = (b: string) => {
    callMobile<{ sessions: Session[] }>("sessions", { badge: b })
  ...
  const open = async (s: Session) => {
    ...
    try {
      ...
      navigate({ to: "/m/count/$id", params: { id: s.id } });
    } catch (e) {
  ...
  return (
    <div className="min-h-dvh bg-background">
      ...
      <div className="px-4 pt-4 pb-10">
        <h1 className="text-lg font-semibold text-foreground">Available Cycle Counts</h1>
        <p className="text-xs text-muted-foreground">Tap a session to start scanning.</p>

        <div className="mt-4 space-y-2">
          {loading && <div className="text-sm text-muted-foreground">Loading…</div>}
          {!loading && sessions.length === 0 && (
            <div className="card-elevated text-center text-sm text-muted-foreground">
              No open cycle counts.
            </div>
          )}
          {sessions.map((s) => {
