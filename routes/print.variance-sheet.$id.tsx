...
export const Route = createFileRoute("/print/variance-sheet/$id")({
  component: PrintVarianceSheet,
...
function PrintVarianceSheet() {
  const { id } = useParams({ from: "/print/variance-sheet/$id" });
  const search = useSearch({ from: "/print/variance-sheet/$id" });
  ...
  const rows = useMemo(() => {
    return items
      .filter(
        (i) =>
          // Hide merged-into-home-bin ghost rows.
    ...
    <div className="min-h-screen bg-white p-8 font-mono text-black print:p-4">
      ...
      <div className="mb-3 flex items-start justify-between border-b-2 border-black pb-2">
        ...
        <div className="text-right text-[11px]">
          <div>Verifier: ____________________</div>
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-b border-gray-400">
              ...
              <td className="px-2 py-0.5 text-right whitespace-nowrap">
                ...
                <span className="inline-block min-w-[36px] border-b border-black">&nbsp;</span>
              </td>
              <td className="px-2 py-0.5 text-center">
                <span className="inline-block min-w-[28px] border-b border-black">&nbsp;</span>
              </td>
            </tr>
          ))}
          {rows.length === 0 && (
